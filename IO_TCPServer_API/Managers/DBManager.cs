...
password TEXT
        ...
        public static bool AddUser(string login, string password, bool bNeedsHashing)
        {
        ...
        public static bool ValidateUser(string login, string pass)
        {
            try
            {
                List<List<string>> result = GetQueryResults("SELECT login, password FROM users WHERE login='" + login + "' AND password='" + pass + "'");
